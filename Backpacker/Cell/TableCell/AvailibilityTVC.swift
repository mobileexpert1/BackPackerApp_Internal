...
class AvailibilityTVC: UITableViewCell {
    ...
    var availableSlots: [Int] = [1, 2, 3]  // Unused slot numbers
    ...
    func addSlot() {
        guard let newSlot = availableSlots.sorted().first else { return }  // Get the smallest available number
        slots.append(newSlot)
        availableSlots.removeAll { $0 == newSlot }
    }
    ...
    @IBAction func action_AddAnotherSlot(_ sender: Any) {
        ...
        guard slots.count < 3 else { return }
            ...
            // Find the first available slot number from 1 to 3
            for i in 1...3 {
                if !slots.contains(i) {
                    slots.append(i)
                    break
...
extension AvailibilityTVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "SlotTVC", for: indexPath) as? SlotTVC else {
            ...
            cell.lbl_timeSlot.text = "Time Slot \(slots[row])"

            // Use captured index
            cell.onDelete = { [weak self] in
                guard let self = self else { return }
                self.slots.remove(at: row)
                if slots.count == 3 {
                    self.VwAnotherSlot_Height.constant = 0.0
                    BgVw_AnotherSlot.isHidden = true
                }else{
